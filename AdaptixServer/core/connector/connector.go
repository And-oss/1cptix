...
func NewTsConnector(ts Teamserver, tsProfile profile.TsProfile, httpServer profile.TsHttpServer) (*TsConnector, error) {
	...
	{
		login_group.POST("/auth/in", connector.tcLogin)
		login_group.POST("/auth/renew", token.RefreshTokenHandler)
	}
	...
	{
		otp_group.POST("/sec/push", connector.tcOTP_UploadTemp)
		otp_group.GET("/sec/pull", connector.tcOTP_DownloadSync)
		otp_group.GET("/ws/open", connector.tcConnectOTP)
		otp_group.GET("/ws/feed", connector.tcChannelOTP)
	}
	...
	{
		api_group.POST("/core/sync", connector.tcSync)
		api_group.POST("/core/sub", connector.tcSubscribe)
		api_group.POST("/sec/gen", connector.tcOTP_Generate)

		api_group.GET("/relay/list", connector.TcListenerList)
		api_group.POST("/relay/new", connector.TcListenerStart)
		api_group.POST("/relay/edit", connector.TcListenerEdit)
		api_group.POST("/relay/stop", connector.TcListenerStop)
		api_group.POST("/relay/pause", connector.TcListenerPause)
		api_group.POST("/relay/resume", connector.TcListenerResume)

		api_group.GET("/node/list", connector.TcAgentList)
		api_group.POST("/node/build", connector.TcAgentGenerate)
		api_group.POST("/node/drop", connector.TcAgentRemove)

		api_group.POST("/node/cmd/file", connector.TcAgentCommandFile)
		api_group.POST("/node/cmd/run", connector.TcAgentCommandExecute)
		api_group.POST("/node/cmd/raw", connector.TcAgentCommandRaw)
		api_group.POST("/node/log/clear", connector.TcAgentConsoleRemove)
		api_group.POST("/node/set/tag", connector.TcAgentSetTag)
		api_group.POST("/node/set/mark", connector.TcAgentSetMark)
		api_group.POST("/node/set/color", connector.TcAgentSetColor)
		api_group.POST("/node/update/data", connector.TcAgentUpdateData)

		api_group.GET("/node/job/list", connector.TcAgentTaskList)
		api_group.POST("/node/job/cancel", connector.TcAgentTaskCancel)
		api_group.POST("/node/job/delete", connector.TcAgentTaskDelete)
		api_group.POST("/node/job/hook", connector.TcAgentTaskHook)
		api_group.POST("/node/job/save", connector.TcAgentTaskSave)

		api_group.POST("/msg/out", connector.TcChatSendMessage)

		api_group.GET("/fetch/list", connector.TcDownloadList)
		api_group.POST("/fetch/sync", connector.TcGuiDownloadSync)
		api_group.POST("/fetch/delete", connector.TcGuiDownloadDelete)

		api_group.GET("/snap/list", connector.TcScreenshotList)
		api_group.GET("/snap/img", connector.TcScreenshotGetImage)
		api_group.POST("/snap/note", connector.TcScreenshotSetNote)
		api_group.POST("/snap/drop", connector.TcScreenshotRemove)

		api_group.GET("/vault/list", connector.TcCredentialsList)
		api_group.POST("/vault/add", connector.TcCredentialsAdd)
		api_group.POST("/vault/edit", connector.TcCredentialsEdit)
		api_group.POST("/vault/drop", connector.TcCredentialsRemove)
		api_group.POST("/vault/tag", connector.TcCredentialsSetTag)

		api_group.GET("/scope/list", connector.TcTargetsList)
		api_group.POST("/scope/add", connector.TcTargetsAdd)
		api_group.POST("/scope/edit", connector.TcTargetEdit)
		api_group.POST("/scope/drop", connector.TcTargetRemove)
		api_group.POST("/scope/tag", connector.TcTargetSetTag)

		api_group.GET("/pipe/list", connector.TcTunnelList)
		api_group.POST("/pipe/new/socks5", connector.TcTunnelStartSocks5)
		api_group.POST("/pipe/new/socks4", connector.TcTunnelStartSocks4)
		api_group.POST("/pipe/new/lportfwd", connector.TcTunnelStartLpf)
		api_group.POST("/pipe/new/rportfwd", connector.TcTunnelStartRpf)
		api_group.POST("/pipe/stop", connector.TcTunnelStop)
		api_group.POST("/pipe/meta", connector.TcTunnelSetIno)

		api_group.GET("/ext/list", connector.TcServiceList)
		//api_group.POST("/ext/load", connector.TcServiceLoad)
		//api_group.POST("/ext/unload", connector.TcServiceUnload)
		api_group.POST("/ext/call", connector.TcServiceCall)
